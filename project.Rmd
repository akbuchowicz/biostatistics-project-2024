...
data.rs1800629.rep <- data.frame(region, G_freq, T_freq)
...
In the beginning we have to prepare data. We are filtering data to get data about healthy patients and then remove nans.
```{r}
data_HC <- data[data$Group == 'HC', ]
head(data_HC)
...
proportion.rs1799724 <- data.frame(
  ...
  G_freq = 1 - proportion.allelesT.rs1799724
)
